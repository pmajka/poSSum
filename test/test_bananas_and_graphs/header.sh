...
OV_SETTINGS=" --output-volume-spacing ${OV_SPACING} --output-volume-origin ${OV_ORIGIN} --output-volume-orientation ${OV_ORIENTATION_CODE}  --output-volume-permute-axes ${OV_PERMUTATION} --output-volume-scalar-type ${OV_TYPE} --output-volume-filp-axes ${OV_FLIP}"
